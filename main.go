...
func main() {
	...
	//user
	newUserRepository := repositories.NewUserRepository(postgresConnection)
	newUserService := services.NewUserService(newUserRepository)
	newUserController := controllers.NewUserController(newUserService)

	//partner
	newPartnerRepository := repositories.NewPartnerRepository(postgresConnection)
	newPartnerService := services.NewPartnerService(newPartnerRepository)
	newPartnerController := controllers.NewPartnerController(newPartnerService)

	//check permission middlewares
	newCheckPermissionMiddleware := middlewares.NewCheckPermissionMiddleware(newUserRepository)
